...
async function findUser(req, res) {
  try {
    ...
    if (req.query.id || req.query.username) {
      ...
      res.json({
        message:
          "The user could not be found or you don't have authorization to view.",
        logtime: new Date(),
      });
    } else {
      res.status(400);
    ...
    res.status(500);
    res.json({
      message: "Something broke on our side, it's probably not your fault",
      logtime: new Date(),
    });
  }
}
...
async function createUser(req, res) {
  ...
  const verifiedData = helpers.verifyRequiredFieldData(requiredFields, {
    payload: req.body,
  });
  if (
    verifiedData.missingOrInvalidFields &&
    verifiedData.missingOrInvalidFields.length > 0
  ) {
    res.status(400);
    res.json({
  ...
  if (
    ...
    try {
      ...
      try {
        ...
        console.error(
          `Error hashing the password for db storage (${password})`,
          err
        );
        throw err;
      }
      ...
      if (req.body.roles && req.body.roles.length > 0) {
        associatedRoles = await usersRolesLogic.buildUserRoleAssociations(
          newUser,
          req.body.roles
        );
      }
...
async function updateUser(req, res) {
  ...
  const verifiedUpdatableData = helpers.verifyRequiredFieldData(
    updatableFields,
    { payload: req.body }
  );
  if (
    verifiedUpdatableData.missingOrInvalidFields &&
    verifiedUpdatableData.missingOrInvalidFields.length > 1
  ) {
    res.status(400);
    res.json({
      message:
        "Request contains no updatable fields. At least one of the updatable fields must be in request",
  ...
  try {
    ...
    if (user) {
      let updatedUser = await userTransactionLogic.updateUser(id, {
        firstname,
        lastname,
      });
      if (updatedUser && roles && Object.keys(roles).length) {
        const updatedRolesReport =
          await usersRolesLogic.updateUserRoleAssociations(updatedUser, roles);
  ...
  if (req.params.id) {
    try {
      ...
      if (deleted) {
        ...
        res.status(400);
        res.json({
          message: "The user was not deleted. Does the user exist?",
          logtime: new Date(),
        });
        // return;
      }
      ...
      res.status(500);
      res.json({
        message: "We have encountered an error trying to delete the user",
        logtime: new Date(),
      });
    }
  }
